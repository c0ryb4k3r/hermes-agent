...
def _settings() -> dict:
    ...
    # Try several common config keys so the value the user already has in config.yaml is picked up
    display_name = (
        setting("HERMES_DASHBOARD_OIDC_PROVIDERNAME", "provider_name")
        or setting("HERMES_DASHBOARD_OIDC_PROVIDERNAME", "display_name")
        or setting("HERMES_DASHBOARD_OIDC_PROVIDERNAME", "name")
        or setting("HERMES_DASHBOARD_OIDC_PROVIDERNAME", "label")
        or "Self-hosted OIDC"
    )
...
def register(ctx) -> None:
    ...
    if kw is not None:
        logger.info(
            "dashboard-auth-self-hosted: registered provider (issuer=%s, client_id=%s, scopes=%r, confidential=%s, display_name=%r)",
            kw.get("issuer"), kw.get("client_id"), kw.get("scopes"), bool(kw.get("client_secret")), kw.get("display_name"))  # never log the secret itself
